...
#define MOVING_SUN
static float outerSphereRad = 3.5;
...
static vec3 sunCol = vec3(258.0, 208.0, 100.0) / 4255.0;
...
static const float nudge = 0.71;	// size of perpendicular vector
...
static vec3 camPos = vec3(0,0,0), camFacing;
static vec3 camLookat=vec3(0,0.0,0);
...
float4 fractalBall(float2 fragCoord)
{
    ...
    // debugging camera
    float mx=iMouse.x/iResolution.x*PI*2.0 + 0*BeatTime * 0.166;
	float my=-iMouse.y/iResolution.y*10.0 + sin(0*BeatTime * 0.3)*0.8+0.1;//*PI/2.01;
    // move camera in and out of the sphere
    float smallTime = 1 + BeatTime*0.15;
    float inOut = pow(abs(-cos(smallTime)), 0.6)* sign(-cos(smallTime));
    inOut *= (1 + pow(frac(BeatTime), 10));
    ...
    {
        rotAmount = PI;
        float c = cos(chunkAnim * rotAmount);
        float s = sin(chunkAnim * rotAmount);
...
#ifdef MOVING_SUN
	vec3 sunDir = normalize(vec3(sin(BeatTime*0.047-1.5), cos(BeatTime*0.047-1.5), -0.5));
#else
    ...
    //if (t <= maxDepth)
	{
        ...
        vec3 posTex = pos;
        if (dot(pos, rotDir) > 1.0) 
            posTex = mul(rotMat, pos);
        ...
        texColor *= smoothstep(texColor, rust, vec3(1,1,1)*(saturate(RustNoise3D(posTex*8.0))-0.2));

        // make outer edge a little brighter
		texColor += (1.0 - vec3(19.0, 5.0, 2.0) * length(normalU))*ambientS;
        // apply noise
        texColor *= vec3(1,1,1)*n*0.05;
        texColor *= 0.7;
        texColor = saturate(texColor);

        // ------ Calculate lighting color ------
        // Start with sun color, standard lighting equation, and shadow
        vec3 lightColor = vec3(0.6,0.6,0.6) * saturate(dot(sunDir, normal)) * sunShadow;
...
//        lightColor *= ambient;
        ...
        finalColor += vec3(1,1,1)*pow(n,4.0)* GetSunColorSmall(ref, sunDir) * sunShadow;// * ambientS;
    ...
    // vignette?
    finalColor *= vec3(1,1,1) * saturate(1.0 - length(uv/2.5));
    finalColor *= 1.3;
