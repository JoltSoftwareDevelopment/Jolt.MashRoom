/****************************************************************************************************
 * original code by skaven
 * https://www.shadertoy.com/view/4tBGWV
 * 
...
static const mat2 m = mat2( 0.80,  0.60, -0.60,  0.80 );
...
float dist_field( vec3 pos ) {
	...
	// object 1 : cube
	float d1 = dist_box( pos, vec3( 2,2,2 ) * boxSize );
		/*
...
{
    float f = 0.0;
    f += 0.5000*noise( p ); p = mul(p*2.02,m); p.y += 0.02*iGlobalTime;
    f += 0.2500*noise( p ); p = mul(p*2.03,m); p.y -= 0.02*iGlobalTime;
    f += 0.1250*noise( p ); p = mul(p*2.01,m); p.y += 0.02*iGlobalTime;
    f += 0.0625*noise( p );
    return f/0.9375;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 offset = vec3(msaa[0] / iResolution.y, 0.);
	vec3 dir = mul(ray_dir( 45.0, iResolution.xy, fragCoord.xy ) + offset, rot);
    camd = dir;
    ...
    if ( depth >= clip_far ) 
    {
        resColor = float4(background(fragCoord), 0);
        //float distToPlan = dot(eye,-camd.xyz);
        vec3 pos = eye + distanceToCube * camd;

        float dist2box = dist_box( pos, vec3(1,1,1)* boxSize*0.66+0.33  );
	...
	else
    {
        ...
        resColor += vec4( max(shading( pos, normalize(mul(-n, rot)), mul(eye,rot) ),0.2) * attn, 1.0 );
    }
    fragColor = resColor;

 //   vec2 tc = fragCoord.xy / iResolution.xy;
	//float4 nisse = resColor * vec4(1,1,1,1)*(Vignette(tc + fbm2(tc*500.0)*0.02)); // remove vignette banding with some noise
 //   fragColor = float4(hsl2rgb(float3(0.6, 0.9, 0.5*length(nisse))), 0);
}
