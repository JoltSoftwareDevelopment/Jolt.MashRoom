...
#define SKIP_HEADER
    #include "otavioGood.symmetricOrigins.ps.hlsl"
#undef SKIP_HEADER
 ...
 *
 ****************************************************************************************************/
static float time=iGlobalTime+25.0;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
  ...
  if (f<maxd){
    ...
    vec3 objColor = hsl2rgb(float3(0.6, 0.9, length(b*c+pow(b,2.0))));
  ...
  } else fragColor=vec4(0, 0, 0, 1);//vec4(background(fragCoord), 1);

  fragColor.rgb = saturate(fragColor.rgb) + background(fragCoord);
}
